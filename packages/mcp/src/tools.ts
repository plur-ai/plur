  ...
  return [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          pin_tier: { type: 'string', enum: ['hard', 'soft'], description: 'Tier within the always-on budget (requires pinned:true). "hard": write-rejected if this single engram exceeds PINNED_HARD_PER_ENGRAM_TOKEN_CAP (200 tokens) or if the aggregate hard-tier total would exceed 2,000 tokens — guaranteed injection on every session. "soft": priority-ordered eviction when soft-tier budget (30% of maxTokens) is exceeded. Default: "soft".' },
          pinned_priority: { type: 'number', description: 'Soft-tier eviction priority 1–100 (default 50, requires pin_tier:"soft" or omitted). Higher value survives eviction longer. Ignored for pin_tier:"hard".' },
      ...
      handler: async (args, plur) => {
        ...
        const context = {
          ...
          pin_tier: args.pin_tier as 'hard' | 'soft' | undefined,
          ...
          engrams: {
            ...
            items: {
              ...
              properties: {
                ...
                pin_tier: { type: 'string', enum: ['hard', 'soft'], description: 'Tier within the always-on budget (requires pinned:true). "hard": write-rejected if single engram exceeds 200 tokens or aggregate exceeds 2,000 tokens. "soft": priority-ordered eviction. Default: "soft".' },
                pinned_priority: { type: 'number', description: 'Soft-tier eviction priority 1–100 (default 50). Higher survives longer. Ignored for pin_tier:"hard".' },
      ...
      handler: async (args, plur) => {
        ...
        const items = raw.map((e) => ({
          ...
          context: {
            ...
            pin_tier: e.pin_tier as 'hard' | 'soft' | undefined,
            pinned_priority: e.pinned_priority as number | undefined,
