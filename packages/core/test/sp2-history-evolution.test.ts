...
describe('SP2 Idea 7: Enhanced Event-Sourced History', () => {
  ...
  it('readHistoryForEngram returns events for specific engram', async () => {
    ...
    const e1 = await plur.learn('First statement')
    const e2 = await plur.learn('Second statement')
  ...
  it('getEngramHistory works via Plur instance', async () => {
    const plur = new Plur({ path: dir })
    const engram = await plur.learn('Test engram')
  ...
  it('forget logs engram_retired in history', async () => {
    const plur = new Plur({ path: dir })
    const engram = await plur.learn('Will be forgotten')
...
describe('SP2 Idea 8: Version Lineage Tracking', () => {
  ...
  it('new engrams start at engram_version 1', async () => {
    const plur = new Plur({ path: dir })
    const engram = await plur.learn('Version test')
    expect((engram as any).engram_version).toBe(1)
  })

  it('new engrams have empty episode_ids', async () => {
    const plur = new Plur({ path: dir })
    const engram = await plur.learn('Episode test')
    expect((engram as any).episode_ids).toEqual([])
  })

  it('status includes versioned_engram_count', async () => {
    const plur = new Plur({ path: dir })
    await plur.learn('Test statement')
...
describe('SP2 Idea 24: Episodic Anchoring', () => {
  ...
  it('auto-links session episode when session_episode_id provided', async () => {
    const plur = new Plur({ path: dir })
    const episode = plur.capture('Test session')

    const engram = await plur.learn('Learned during session', {
  ...
  it('does not link episodes when session_episode_id is not provided', async () => {
    const plur = new Plur({ path: dir })
    const engram = await plur.learn('No session context')
    expect((engram as any).episode_ids).toEqual([])
  })
})
