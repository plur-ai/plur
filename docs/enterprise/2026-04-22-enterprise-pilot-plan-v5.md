# PLUR Enterprise Pilot — Implementation Plan (v5, final)
...
**Security:** 62 findings from 3 review rounds (security audit, pen-test, architecture, QA) addressed inline in task code.
...
Task 10:  Auth (GitLab OAuth callback, dual-secret JWT rotation, startup 503 gate)
...
Task 6:   TenantManager + migration CLI ──────────────┤
                                                       ▼
Task 7:   GitLab client (HTTPS, Zod, pagination) ────┤
Task 7b:  GitLab OAuth + token encryption ────────────┤ (parallel with 7)
Task 8:   GitLab sync + ensureUserSynced + CLI ───────┤ (requires 5+7)
Task 9:   GitLab webhooks (timingSafeEqual, replay) ──┤ (requires 5+7)
                                                       ▼
Task 10:  Auth (GitLab OAuth, dual-secret JWT, 503) ──┤ (requires 7b+8)
...
2. Tasks 4+4b+5+6 (data layer) — parallel after 2+3. Task 4b is a PR to the plur repo.
3. Tasks 7+7b (GitLab client + OAuth) — parallel after 2+3
4. Tasks 8+9 (sync + webhooks) — parallel after 5+7
5. Tasks 11+12+13 (server layer) — can start after 10, but 11 also needs 4b merged
6. Task 13b (tool allowlist) — requires 13, critical path before security tests
7. Tasks 14+15+16 — sequential after all implementation
...
## Security Evaluator Findings — Resolution Matrix (v5 task numbers)
