...
describe('intent routing — recall integration', () => {
  ...
  beforeEach(async () => {
    ...
    await plur.learn('Yesterday we deployed the new auth service', { type: 'behavioral' })
    await plur.learn('This morning Karl confirmed the contract terms', { type: 'behavioral' })
    await plur.learn('Karl Schmidt works at Acme as the head of sales', {
      type: 'terminological',
      domain: 'crm.contacts',
    })
    await plur.learn('Karl prefers email over phone for outreach', {
      type: 'behavioral',
      domain: 'crm.contacts',
    })
  })
  ...
  it('recall returns results unchanged when intent routing OFF (env opt-out)', async () => {
    ...
    try {
      const results = await plur.recall('Karl')
  ...
  it('accepts intentOverride option', async () => {
    ...
    const results = await plur.recall('Karl', { intentOverride: 'entity' } as any)
    expect(Array.isArray(results)).toBe(true)
  })

  it('entity-intent recall surfaces entity-domain engrams (crm.contacts)', async () => {
    const results = await plur.recall("Karl's email", { intentOverride: 'entity' } as any)
    expect(results.length).toBeGreaterThan(0)
  ...
  it('intent routing does not silently drop results — wrong intent still returns hits', async () => {
    // Even with mismatched intent, the query should still recall results
    const r1 = await plur.recall('PostgreSQL database', { intentOverride: 'temporal' } as any)
    const r2 = await plur.recall('PostgreSQL database', { intentOverride: 'general' } as any)
  ...
  it('default behavior is intent routing ON', async () => {
    ...
    const results = await plur.recall('Karl email')
    expect(results.length).toBeGreaterThan(0)
  })
