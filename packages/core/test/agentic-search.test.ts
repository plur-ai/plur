...
describe('agentic search', () => {
  ...
  beforeEach(async () => {
    ...
    await plur.learn('Meeting scheduled for Friday at 3pm', { type: 'behavioral' })
    await plur.learn('Python is used for data analysis scripts', { type: 'procedural' })
  ...
  it('returns sync results in fast mode (default)', async () => {
    const result = await plur.recall('database')
