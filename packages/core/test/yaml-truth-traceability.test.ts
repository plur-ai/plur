...
describe('yaml-as-truth: public API traceability (Test B)', () => {
  ...
  beforeEach(async () => {
    ...
    await plur.learn('verify dates with datacore.date', {
      scope: 'project:plur',
    ...
    await plur.learn('the user prefers terse responses', {
      scope: 'global',
      domain: 'workflow.communication',
      type: 'behavioral',
    })
    await plur.learn('embedding choice dominates fusion choice', {
      scope: 'project:plur',
  ...
  it('list() returns only engrams present in YAML', async () => {
    const truth = yamlGroundTruth(dir)
    const returned = await plur.list()
  ...
  it('recall() returns only engrams present in YAML', async () => {
    const truth = yamlGroundTruth(dir)
    const returned = await plur.recall('source of truth')
    expect(returned.length).toBeGreaterThan(0)
    for (const e of returned) {
      expect(truth.has(e.id),
  ...
  it('inject() returns only engrams present in YAML', async () => {
    const truth = yamlGroundTruth(dir)
    const result = await plur.inject('about to merge a PR')
  ...
  it('getById() returns only engrams present in YAML', async () => {
    const truth = yamlGroundTruth(dir)
    const ids = Array.from(truth)
    for (const id of ids) {
      const e = await plur.getById(id)
      expect(e).not.toBeNull()
      expect(truth.has(e!.id)).toBe(true)
    }
  })

  it('getById() returns null for ids not in YAML', async () => {
    expect(await plur.getById('ENG-9999-9999-999')).toBeNull()
  })

  it('list with scope filter returns only YAML-backed engrams', async () => {
    const truth = yamlGroundTruth(dir)
    const returned = await plur.list({ scope: 'project:plur' })
    for (const e of returned) {
      expect(truth.has(e.id)).toBe(true)
    }
  })

  it('every returned engram has its YAML-derived fields intact', async () => {
    const truth = yamlGroundTruth(dir)
    const yamlEngrams = loadEngrams(join(dir, 'engrams.yaml'))
    const byId = new Map(yamlEngrams.map(e => [e.id, e]))

    for (const returned of await plur.list()) {
